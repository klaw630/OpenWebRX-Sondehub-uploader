...
class SondehubUploader(object):
    ...
    def __init__(
        ...
        upload_timeout=20,
        upload_retries=5,
        user_callsign="HA1NB-TEST",
        user_position=(47.29, 16.69, 216),
        user_antenna="",
        contact_email="",
    ...
    def reformat_data(self, telemetry):
        ...
        _output = {
            "software_name": "OpenWebRX+ HA1NB Uploader ",
...
if __name__ == "__main__":
    ...
    if not config.sections():
        ...
        for line in open(CONFIG_FILE):
            if "=" in line:
                ...
                config["station"][key.strip()] = val.strip()

    station_cfg = config["station"]

    CALLSIGN = station_cfg.get("uploader_callsign", "NOCALL")
    LAT = float(station_cfg.get("station_lat", 0.0))
    LON = float(station_cfg.get("station_lon", 0.0))
    ALT = float(station_cfg.get("station_alt", 0.0))
    ANTENNA = station_cfg.get("uploader_antenna", "")
    UPLOAD_RATE = int(station_cfg.get("sondehub_upload_rate", 30))
    CONTACT_EMAIL = station_cfg.get("sondehub_contact_email", "")
    ...
    logging.info(f"SondeHub uploader started for {CALLSIGN} at ({LAT}, {LON}, {ALT})")
    logging.info("Reading JSON telemetry from stdin...")
    ...
    def process_line(line):
        try:
            ...
            if "datetime" in pkt:
                pkt["datetime_dt"] = dateparser.isoparse(pkt.pop("datetime"))
            else:
                pkt["datetime_dt"] = time.gmtime()
            if "type" not in pkt:
                pkt["type"] = "RS41"
            uploader.add(pkt)
        except Exception as e:
            logging.error(f"Failed to process line: {line} - {e}")
